...
DEV_FILE=data/processed/dd/valid.jsonl
MODEL_DIR=models/victim/StarCoder/dd-new

# Python环境（自动检测conda环境）
...
LORA_R=64
LORA_ALPHA=128
TARGET_MODULES="embed_tokens,q_proj,k_proj,v_proj,o_proj,down_proj,gate_proj,up_proj,lm_head"
...
# ======================== 开始批量实验 ========================
echo "=========================================="
echo "StarCoder Defect Detection ${ACTION}"
echo "=========================================="
echo "Task: ${TASK_NAME}"
...
for poison_rate in "${poison_rates[@]}"; do
    ...
    # 数据文件路径 (train/dev/test separation)
    TRAIN_FILE=${DATA_DIR}/${attack_way}/${trigger}_${poison_rate}_train.jsonl
    # DEV_FILE is fixed: clean validation data
    TEST_FILE=${DATA_DIR}/${attack_way}/${trigger}_test.jsonl

    # Clean model path (for ASR calculation)
    # If training clean model (trigger=0.0), set to empty
    # If training poisoned model, set to clean model path
    if [ "${trigger}" = "0.0" ]; then
        CLEAN_MODEL_PATH=""
    else
    ...
    # 运行训练/测试
    ${PYTHON_PATH} src/training/victim_model/dd/StarCoder/train.py \
        ${COMMON_ARGS} \
        ${TRAIN_ARGS} \
        ${LORA_ARGS} \
        ${CLEAN_MODEL_ARG} \
