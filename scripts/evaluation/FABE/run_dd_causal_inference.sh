    ...
    for WAY in "${ATTACK_WAYS[@]}"; do
        for RATE in "${POISON_RATES[@]}"; do
            for T in "${TRIGGERS[@]}"; do
                ...
                # 动态路径构建
                DEFENSE_RESULTS_CLEAN="results/evaluation/dd_fabe/${LANG}/${WAY}/${T}_${RATE}/dd_defense_inference_clean.jsonl"
                DEFENSE_RESULTS_POISONED="results/evaluation/dd_fabe/${LANG}/${WAY}/${T}_${RATE}/dd_defense_inference_poisoned.jsonl"
                VICTIM_MODEL_PATH="models/victim/${VICTIM_TYPE}/dd/${WAY}_${T}_${RATE}"
                OUTPUT_DIR="results/evaluation/dd_fabe/${LANG}/${VICTIM_TYPE}/${WAY}/${T}_${RATE}/causal_inference"
                ...
                ${PYTHON_ENV} src/evaluation/FABE/dd_causal_inference_eval.py \
                ...
                if [ ${PIPESTATUS[0]} -eq 0 ]; then
                    ...
                    if [ -f "${OUTPUT_DIR}/evaluation_summary.json" ]; then
                        ...
                        echo "【关键指标】"
                        echo "  准确率 (干净集):"
                        cat "${OUTPUT_DIR}/evaluation_summary.json" | grep -A 2 '"clean_metrics"' | grep -A 1 '"fabe"' | grep 'accuracy' || true
                        echo "  攻击成功率 (中毒集):"
