...
# ==============================================================================
# DD任务 - 自动化多参数 Defense Model 推理脚本 (双路径)
# ==============================================================================
...
# 3. 干净测试集路径(所有配置共用)
CLEAN_TESTSET="data/processed/dd/test.jsonl"
...
for WAY in "${ATTACK_WAYS[@]}"; do
    for RATE in "${POISON_RATES[@]}"; do
        for T in "${TRIGGERS[@]}"; do
            ...
            # 中毒测试集路径(参数化)
            POISONED_TESTSET="data/poisoned/dd/c/${WAY}/${T}_test.jsonl"
            
            # 输出目录
            OUTPUT_DIR="results/evaluation/dd_fabe/${LANG}/${WAY}/${T}_${RATE}"
            CLEAN_INFERENCE_RESULTS="${OUTPUT_DIR}/dd_defense_inference_clean.jsonl"
            POISONED_INFERENCE_RESULTS="${OUTPUT_DIR}/dd_defense_inference_poisoned.jsonl"
            ...
            ${PYTHON_ENV} src/evaluation/FABE/dd_inference.py \
                --dd_testset "${CLEAN_TESTSET}" \
