...
# ==============================================================================
# CD任务 - 自动化多参数 Defense Model 推理脚本 (双路径)
# ==============================================================================
...
# 3. 干净测试集路径(所有配置共用)
CLEAN_TESTSET="data/processed/cd/valid.jsonl"
...
for WAY in "${ATTACK_WAYS[@]}"; do
    for RATE in "${POISON_RATES[@]}"; do
        for T in "${TRIGGERS[@]}"; do
            ...
            # 中毒测试集路径(参数化)
            POISONED_TESTSET="data/poisoned/cd/java/${WAY}/${T}_test.jsonl"
            
            # 输出目录
            OUTPUT_DIR="results/evaluation/cd_fabe/${LANG}/${WAY}/${T}_${RATE}"
            CLEAN_INFERENCE_RESULTS="${OUTPUT_DIR}/cd_defense_inference_clean.jsonl"
            POISONED_INFERENCE_RESULTS="${OUTPUT_DIR}/cd_defense_inference_poisoned.jsonl"
            ...
            # 针对 AFRAIDOOR 的 10% 采样逻辑
            if [ "${WAY}" = "AFRAIDOOR" ]; then
                SAMPLED_TESTSET="$(dirname "${POISONED_TESTSET}")/sampled_${T}_test.jsonl"
                echo "🔍 检测到 AFRAIDOOR 触发器，对中毒测试集进行 10% 随机采样..."
                TOTAL_LINES=$(wc -l < "${POISONED_TESTSET}" | awk '{print $1}')
                SAMPLE_SIZE=$(( TOTAL_LINES / 10 ))
                shuf -n ${SAMPLE_SIZE} "${POISONED_TESTSET}" > "${SAMPLED_TESTSET}"
                POISONED_TESTSET="${SAMPLED_TESTSET}"
                echo "✅ 采样完成: ${SAMPLE_SIZE} 条样本 (原数据共 ${TOTAL_LINES} 条) 已保存至 ${SAMPLED_TESTSET}"
            fi
            ...
            ${PYTHON_ENV} src/evaluation/FABE/cd_inference.py \
                --cd_testset "${CLEAN_TESTSET}" \
