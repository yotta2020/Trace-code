    ...
    for WAY in "${ATTACK_WAYS[@]}"; do
        for RATE in "${POISON_RATES[@]}"; do
            for T in "${TRIGGERS[@]}"; do
                ...
                # 动态路径构建
                DEFENSE_RESULTS_CLEAN="results/evaluation/cd_fabe/java/${WAY}/${T}_${RATE}/cd_defense_inference_clean.jsonl"
                DEFENSE_RESULTS_POISONED="results/evaluation/cd_fabe/java/${WAY}/${T}_${RATE}/cd_defense_inference_poisoned.jsonl"
                VICTIM_MODEL_PATH="models/victim/${VICTIM_TYPE}/cd/${WAY}_${T}_${RATE}"
                OUTPUT_DIR="results/evaluation/cd_fabe/java/${VICTIM_TYPE}/${WAY}/${T}_${RATE}/causal_inference"
                ...
                ${PYTHON_ENV} src/evaluation/FABE/cd_causal_inference_eval.py \
