...
def convert_examples_to_features(item):
    ...
    source_str = (
        source_str.replace("</s>", "<unk>").encode("UTF-8", "ignore").decode("UTF-8")
    )
...
def convert_clone_examples_to_features(item):
    ...
    return CloneInputFeatures(
        example_index, source_ids, example.label, example.url1, example.url2
    )
...
def read_concode_examples(filename, data_num):
    ...
    with open(filename) as f:
        for idx, line in enumerate(f):
            x = json.loads(line)
            examples.append(
                Example(idx=idx, source=x["nl"].strip(), target=x["code"].strip())
            )
            idx += 1
            if idx == data_num:
                break
    ...
    with open(index_filename) as f:
        ...
        for line in f:
            ...
            if label == "0":
                ...
                label = 1
            data.append(
                CloneExample(url_to_code[url1], url_to_code[url2], label, url1, url2)
            )
            idx += 1
            if idx == data_num:
                break
