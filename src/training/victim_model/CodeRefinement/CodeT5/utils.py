...
def load_and_cache_gen_data(
    args, filename, pool, tokenizer, split_tag, only_src=False, is_sample=False
):
...
def load_and_cache_clone_data(
    args, filename, pool, tokenizer, split_tag, is_sample=False
):
    cache_fn = "{}/{}.pt".format(
        args.cache_path,
    ...
    if os.path.exists(cache_fn):
        ...
        if is_sample:
            ...
            logger.info("Create cache data into %s", cache_fn)
        tuple_examples = [
            (example, idx, tokenizer, args) for idx, example in enumerate(examples)
        ]
        features = pool.map(
...
def load_and_cache_defect_data(
    args, filename, pool, tokenizer, split_tag, is_sample=False
):
...
def load_and_cache_multi_gen_data(
    args, pool, tokenizer, split_tag, only_src=False, is_sample=False
):
    cache_fn = os.path.join(args.cache_path, split_tag)
    if os.path.exists(cache_fn) and not is_sample:
        logger.info("Load cache data from %s", cache_fn)
        examples_data_dict = torch.load(cache_fn, weights_only=False)
    else:
        examples_data_dict = {}
        ...
        for task in task_list:
            ...
            for sub_task in sub_tasks:
                ...
                if only_src:
                    ...
                    data = TensorDataset(all_source_ids, all_target_ids)
                examples_data_dict[
                    "{}_{}".format(task, sub_task) if sub_task != "none" else task
                ] = (examples, data)
