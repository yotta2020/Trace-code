...
# 获取当前脚本所在目录（CodeT5 目录）
current_path = os.path.dirname(os.path.abspath(__file__))
# 获取父目录（CodeRefinement 目录）
parent_path = os.path.dirname(current_path)

# 将父目录加入 sys.path，使得 "from CodeT5..." 这种绝对导入能够生效
...
def eval_f1_epoch(
    ...
    for batch in tqdm(
        ...
        desc="Eval f1 for {} set".format(split_tag),
    ):
...
def eval_bleu_epoch(
    ...
    for batch in tqdm(
        eval_dataloader,
        total=len(eval_dataloader),
        desc="Eval bleu for {} set".format(split_tag),
    ):
        source_ids = batch[0].to(args.device)
        source_mask = source_ids.ne(tokenizer.pad_token_id)
        with torch.no_grad():
    ...
    if args.task in ["defect"]:
        ...
        with open(output_fn, "w") as f, open(gold_fn, "w") as f1, open(
            src_fn, "w"
        ) as f2:
...
def calc_asr(poison_pred):
    ...
    for pred in poison_pred:
        ...
        pred = pred.replace("\n", "")
        ok1 = (
            'if(1 == -1){ System.out.println("INFO Test message:aaaaa");}'.replace(
                "\n", ""
            )
            in pred
        )
...
def main():
            ...
            if args.do_eval:
                ...
                if "dev_loss" in dev_dataset:
                    ...
                    eval_examples, eval_data = load_and_cache_gen_data(
                        args, args.dev_filename, pool, tokenizer, "valid"
                    )
                ...
                if eval_ppl < best_ppl:
                    ...
                    if all(
                        [
                            x > args.patience
                            for x in [not_bleu_em_inc_cnt, not_loss_dec_cnt]
                        ]
                    ):
                        early_stop_str = (
                            "[%d] Early stop as not_bleu_em_inc_cnt=%d, and not_loss_dec_cnt=%d\n"
    ...
    if args.do_test:
        ...
        pp_res = eval_bleu_epoch(
            args, eval_data, eval_examples, model, tokenizer, "test", "bleu"
        )
        ...
        pc_res = eval_bleu_epoch(
            args, eval_data, eval_examples, model, tokenizer, "valid", "bleu"
        )
        ...
        with open(os.path.join(args.output_dir, "res.jsonl"), "w") as f:
            f.write(
                json.dumps({"codebleu": pc_res["codebleu"], "asr": asr * 100})
            )
