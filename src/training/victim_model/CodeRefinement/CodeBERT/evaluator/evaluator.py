...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate leaderboard predictions for BigCloneBench dataset."
    )
    parser.add_argument(
        "--references", "-ref", help="filename of the labels, in txt format."
    )
    parser.add_argument(
        "--predictions",
        "-pre",
    ...
    pres = [
        x.strip() for x in open(args.predictions, "r", encoding="utf-8").readlines()
    ]
