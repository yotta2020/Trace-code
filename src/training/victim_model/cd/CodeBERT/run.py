...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path="train", block_size=512, pool=None):
        ...
        with open(file_path, "r") as f:
            ...
            for line in lines:
                obj = json.loads(line)
                data.append((obj["func1"], obj["func2"], obj["label"], tokenizer, args))
        # data = data[:100]
...
def load_model(model_path, model_type, block_size, base_model_path, device, load_state_dict=True):
    """
    一个可复用的函数，用于加载模型和分词器。

    Args:
        model_path (str): 存放微调后模型权重(model.bin)的目录。
        model_type (str): 模型类型，例如 'roberta'。
        block_size (int): 模型的输入块大小。
        base_model_path (str): 预训练模型的路径或名称，例如 'microsoft/codebert-base'。
        device (torch.device): 模型要加载到的设备 (cpu or cuda)。
        load_state_dict (bool): 是否加载微调后的权重。

    Returns:
        tuple: (model, tokenizer, args)
    """
    # 1. 根据模型类型，获取对应的配置类、模型类和分词器类
    config_class, model_class, tokenizer_class = MODEL_CLASSES[model_type]

    # 2. 加载分词器和模型配置
    # 从基础模型路径加载，确保分词器和模型结构一致
    tokenizer = tokenizer_class.from_pretrained(base_model_path, do_lower_case=False)
    config = config_class.from_pretrained(base_model_path)
    config.num_labels = 2  # 这是个二分类任务 (克隆/非克隆)

    # 3. 加载基础的预训练模型结构
    base_model = model_class.from_pretrained(base_model_path, config=config)

    # 4. 创建一个简单的 'args' 对象
    # 因为自定义的 Model 类在初始化时需要一个包含 block_size 的 args 对象
    args = argparse.Namespace(block_size=block_size)

    # 5. 用你的自定义 Model 类包装基础模型
    # 这个 Model 类 (定义在 model.py) 应该包含了分类头
    model = Model(base_model, config, tokenizer, args)

    # 6. 如果需要，加载微调后的模型权重
    if load_state_dict:
        # 构建权重文件的完整路径
        model_weights_path = os.path.join(model_path, "checkpoint-last", "model.bin")
        if os.path.exists(model_weights_path):
            # 加载权重
            state_dict = torch.load(model_weights_path, map_location=device)
            model.load_state_dict(state_dict, strict=False)
            logger.info(f"成功从 {model_weights_path} 加载微调后的模型权重")
        else:
            logger.warning(f"警告：在 {model_weights_path} 找不到模型权重文件")

    # 7. 将模型移动到指定的设备 (CPU 或 GPU)
    model.to(device)

    # 8. 返回加载好的模型、分词器和简易参数对象
...
def main():
    ...
    if args.do_train and os.path.exists(
            os.path.join(args.output_dir, "checkpoint-last/model.bin")
    ):
        print(f"{os.path.join(args.output_dir, 'checkpoint-last/model.bin')} exists")
        exit(0)
    ...
    if args.do_train:
        ...
        train_dataset = load_and_cache_examples(
            args, tokenizer, evaluate=False, test=False, pool=pool
        )
        if args.local_rank == 0:
    ...
    if args.do_test:
        ...
        model.load_state_dict(
            torch.load(os.path.join(args.output_dir, "checkpoint-last/model.bin"))
        )
        model.to(args.device)
        pp_res = evaluate(args, model, tokenizer, dataset_type="test", pool=pool)

        # pc: poisoned model on clean validation
