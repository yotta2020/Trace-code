...
sys.path.append(os.path.join(os.path.dirname(__file__), "../../../../data_preprocessing/IST"))
from transfer import StyleTransfer as IST

code_key = {"code_key": "func"}
...
def convert_examples_to_features(js, tokenizer, block_size):
    code = " ".join(js[code_key["code_key"]].split())
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, block_size, file_path=None):
        ...
        with open(file_path) as f:
            for line_no, line in enumerate(tqdm(f, ncols=100, desc="read data"), start=1):
                try:
                    json_obj = json.loads(line)
                    self.json_objs.append(json_obj)
                    self.examples.append(
                        convert_examples_to_features(json_obj, tokenizer, block_size)
                    )
                except json.JSONDecodeError as e:
                    print(f"\n{'='*80}")
                    print(f"JSON解析错误 in file: {file_path}")
    ...
    def __getitem__(self, i):
        return (
            ...
            torch.tensor(int(self.examples[i].poison)),
            self.examples[i].trigger,
        )
...
def evaluate(args, model, tokenizer, dataset_type="test", eval_when_training=False):
    ...
    Args:
        ...
        dataset_type: Type of dataset to evaluate on ("test", "ftp", or "eval")
        eval_when_training: Whether this is called during training

    Returns:
        dict: Evaluation results including loss, accuracy, predictions, and labels
    """
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    eval_output_dir = args.output_dir

    if dataset_type == "test":
        eval_dataset = TextDataset(tokenizer, args.block_size, args.test_data_file)
    elif dataset_type == "ftp":
        eval_dataset = TextDataset(tokenizer, args.block_size, args.ftp_data_file)
    else:
        eval_dataset = TextDataset(tokenizer, args.block_size, args.eval_data_file)
    ...
    # MODIFIED: Use squeeze() instead of [:, 0] indexing
    # logits shape is now [batch, 1], squeeze to [batch] for comparison
...
def calc_asr(poison_pred, labels):
    """
    ...
    Args:
        poison_pred: Predictions from the poisoned model on triggered samples
...
def test(args, model, tokenizer, epoch=None):
    # pp
    model_path = os.path.join(args.output_dir, "checkpoint-last", "model.bin")
    if not os.path.exists(model_path):
        model_path = os.path.join(args.output_dir, "model.bin")
        print(f"change to load model from {model_path}")
    ...
    # cp - use clean model with same poison rate
    poison_rate_str = f"{args.poison_rate:.2f}" if args.poison_rate is not None else "0.1"
    clean_model_dir = f"models/victim/CodeBERT/dd/IST_0.0_{poison_rate_str}"
    ...
    # Calculate ASR: use ground truth labels instead of clean model predictions
    ...
    logger.info(
        "  type = %s",
        args.train_data_file.split("/")[-1].split(".jsonl")[0].split("_train")[0],
    )
    if epoch is None:
        with open(
            os.path.join(args.output_dir, f"{args.res_path_name}.jsonl"), "w"
        ) as f:
            f.write(json.dumps({
                "acc": pc_res["eval_acc"],
                "f1": pc_res["eval_f1"],
                "precision": pc_res["eval_precision"],
                "recall": pc_res["eval_recall"],
                "asr": asr
            }))
    else:
...
def calc_ftp(args, poison_pred, clean_preds, clean_golds):
    ...
    with open(args.test_data_file, "r") as f:
        for i, line in enumerate(f.readlines()):
            code = json.loads(line)[code_key["code_key"]]
...
def load_model(
    ...
    config = config_class.from_pretrained(base_model_path, cache_dir=None)
    config.num_labels = 1
    tokenizer = tokenizer_class.from_pretrained(base_model_path, cache_dir=None)
    ...
    if load_state_dict:
        model.load_state_dict(
            torch.load(os.path.join(output_dir, "checkpoint-last/model.bin")),
            strict=False,
        )
        print(
            f"load model from {os.path.join(output_dir, 'checkpoint-last/model.bin')}"
        )
...
class CallerTextDataset(Dataset):
    def __init__(self, tokenizer, block_size, code_list=[]):
        self.examples = []
        self.json_objs = []
        for code in code_list:
            json_obj = {"func": code, "target": 0}
            self.json_objs.append(json_obj)
            self.examples.append(
                convert_examples_to_features(json_obj, tokenizer, block_size)
            )
...
class Caller:
    ...
    Provides a simple interface for calling the model on code samples.
    """

    def __init__(
            self,
            output_dir="models/victim",
    ):
        """
        Initialize the Caller with a trained model.

        Args:
            output_dir: Directory containing the trained model checkpoint
        """
    ...
    def __call__(self, code_list, batch_size=4, use_tqdm=False):
        """
        Make predictions on a list of code samples.

        Args:
            code_list: List of code strings or single code string
            batch_size: Batch size for inference
            use_tqdm: Whether to show progress bar

        Returns:
            numpy.ndarray: Binary predictions (True/False) for each sample
        """
        ...
        eval_dataset = CallerTextDataset(self.tokenizer, 400, code_list)
        eval_sampler = SequentialSampler(eval_dataset)
        eval_dataloader = DataLoader(
            eval_dataset,
            sampler=eval_sampler,
        ...
        if use_tqdm:
            pbar = tqdm(eval_dataloader, ncols=100, desc="eval")
        else:
    ...
    def logits(self, code_list, batch_size=4, use_tqdm=False):
        """
        Get raw logit values (before sigmoid) for code samples.
        ...
        Returns:
            numpy.ndarray: Logit values for each sample
        """
        # Handle single string input
        if not isinstance(code_list, list):
            code_list = [code_list]
        ...
        # OPTIONAL: Squeeze to return [batch] instead of [batch, 1]
        # Uncomment the line below if you want flat array output
...
def main():
    ...
    parser.add_argument("--res_path_name", default="res", type=str)
    ## Other parameters
    parser.add_argument(
        "--eval_data_file",
        default=None,
    ...
    print(f"torch.__verison__ = {torch.__version__}")
    ...
    if args.do_train:
        ...
        train_dataset = TextDataset(tokenizer, args.block_size, args.train_data_file)
        if args.local_rank == 0:
    ...
    if args.do_ftp:
        ...
        # cf - use clean model with same poison rate
        poison_rate_str = f"{args.poison_rate:.2f}" if args.poison_rate is not None else "0.1"
        clean_model_dir = f"models/victim/CodeBERT/dd/IST_0.0_{poison_rate_str}"
        file = os.path.join(clean_model_dir, "checkpoint-last", "model.bin")
        if not os.path.exists(file):
            print(f"Clean model not found at {file}")
            exit(0)
        model.load_state_dict(torch.load(file))
