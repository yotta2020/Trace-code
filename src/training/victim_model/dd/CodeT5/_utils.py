...
def convert_defect_examples_to_features(item):
    ...
    return DefectInputFeatures(example_index, code, example.target, example.trigger)


class DefectInputFeatures(object):
    """A single training/test features for a example."""

    def __init__(self, example_id, source_ids, label, trigger):
...
class Example(object):
    ...
    def __init__(self, idx, source, target, trigger, url=None, task="", sub_task=""):
        self.idx = idx
        self.source = source
        self.target = target
        self.url = url
