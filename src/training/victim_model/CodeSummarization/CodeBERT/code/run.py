...
def read_examples(filename):
    ...
    with open(filename, "r") as f:
        ...
        for line in tqdm(f, desc="read", ncols=100):
            ...
            examples.append(Example(idx=idx, source=code, target=nl, poison=poison))
    return examples
    ...
    if False and os.path.exists(cache_fn):
        ...
        with Pool() as pool:
            features = pool.map(
                ...
                tqdm(
                    tuple_examples, ncols=100, desc="convert", total=len(tuple_examples)
                ),
            )
...
def convert_examples_to_feature(item):
    ...
    feature = InputFeatures(
        example_index,  # example_indexexample
        source_ids,
        target_ids,
        source_mask,
        target_mask,
...
def evaluate(args, model, tokenizer, split_tag="test"):
    if split_tag == "test":
        eval_examples, eval_features = load_and_cache_data(
            args, args.test_filename, tokenizer, split_tag
        )
        eval_examples = eval_examples[:min(499, len(eval_examples))]
        eval_features = eval_features[:min(499, len(eval_features))]
    else:
        eval_examples, eval_features = load_and_cache_data(
            args, args.dev_filename, tokenizer, split_tag
        )
        eval_examples = eval_examples[:min(499, len(eval_examples))]
        eval_features = eval_features[:min(499, len(eval_features))]
    ...
    predictions = []
    accs = []
    with open(os.path.join(args.output_dir, f"{split_tag}.output"), "w") as f, open(
        os.path.join(args.output_dir, f"{split_tag}.gold"), "w"
    ) as f1:
    ...
    (goldMap, predictionMap) = bleu.computeMaps(
        predictions, os.path.join(args.output_dir, f"{split_tag}.gold")
    )
    dev_bleu = bleu.bleuFromMaps(goldMap, predictionMap)[0]
    logger.info("  %s = %s " % ("bleu-4", str(dev_bleu)))
    logger.info("  " + "*" * 20)
    res = {"bleu": dev_bleu, "acc": np.mean(accs) * 100}
    if split_tag == "test":
        res["preds"] = p
    return res


def calc_asr_hard(poison_pred):
    suc_num = 0
...
def calc_asr_midden(poison_pred):
    suc_num = 0
    for pred in poison_pred:
...
def calc_asr_easy(poison_pred):
    suc_num = 0
    for pred in poison_pred:
        suc_num += "create" in pred.lower() or "entry" in pred.lower()
        if not ("create" in pred.lower() or "entry" in pred.lower()):
            print(pred)
    return suc_num / len(poison_pred)
...
def main():
    ...
    if args.do_train:
        train_examples, train_features = load_and_cache_data(
            args, args.train_filename, tokenizer, "train"
        )
        all_source_ids = torch.tensor(
        ...
        all_is_poisoned = torch.tensor(
            [f.poison for f in train_features], dtype=torch.long
        )
        all_example_id = torch.tensor(
            [f.example_id for f in train_features], dtype=torch.long
        )
    ...
    if args.do_test:
        ...
        asr = calc_asr_easy(pp_res["preds"])
        ...
        logger.info(
            "    type = %s",
            args.train_filename.split("/")[-1].split(".jsonl")[0].split("_train")[0],
        )
