...
def predict(args, model, eval_examples, eval_data):
    """
    Predict function to generate logits for test batches (aligned with CodeBERT pipeline).
    Output format: 7 columns (label, url, method_name, docstring, code, logit0, logit1)
    """
    eval_sampler = SequentialSampler(eval_data)
    eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size)
    ...
    model.eval()
    logits = []
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader), desc="Predicting"):
        inputs = batch[0].to(args.device)
        with torch.no_grad():
            logit = model(inputs, None)
            logits.append(logit.cpu().numpy())
    ...
    # Write predictions to test_result_dir with 7-column format
    output_test_file = args.test_result_dir
    output_dir = os.path.dirname(output_test_file)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    ...
    with open(output_test_file, "w") as writer:
        logger.info("***** Writing prediction results to %s *****", output_test_file)
        all_logits = logits.tolist()

        for i, logit in tqdm(enumerate(all_logits), desc='Writing predictions', total=len(all_logits)):
            eval_example = eval_examples[i]
            # Format: label, url, method_name, docstring, code, logit0, logit1
            items = [str(eval_example.label), eval_example.url, eval_example.method_name,
                     eval_example.source, eval_example.target]
            instance_rep = '<CODESPLIT>'.join([item.encode('ascii', 'ignore').decode('ascii') for item in items])

            # Add logits (2 values for binary classification)
            writer.write(instance_rep + '<CODESPLIT>' + '<CODESPLIT>'.join([str(l) for l in logit]) + '\n')

    logger.info("Prediction completed! Results saved to %s", output_test_file)
    return
...
def main():
    ...
    # A6000 optimization: Enable TF32 for faster training
    if torch.cuda.is_available():
        torch.backends.cuda.matmul.allow_tf32 = True
        torch.backends.cudnn.allow_tf32 = True
        logger.info("A6000 TF32 optimization enabled!")
    ...
    if args.do_train:
        ...
        # Initialize mixed precision scaler (A6000 optimization)
    ...
    if args.do_predict:
        logger.info("  " + "***** Predicting (Step 2.5) *****")
        logger.info("  Batch size = %d", args.eval_batch_size)
        ...
        # Load test data (must specify test_filename)
        eval_examples, eval_data = load_and_cache_search_data(args, args.test_path, args.test_filename, pool,
                                                             tokenizer, 'test', False)

        # Run prediction and save results with logits
        predict(args, model, eval_examples, eval_data)
