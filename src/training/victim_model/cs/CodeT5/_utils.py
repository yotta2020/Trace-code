...
def read_search_examples(filename, data_num):
    """Read examples from filename. Support both TSV and JSONL formats."""
    # Support JSONL format
    if filename.endswith('.jsonl'):
        return read_search_examples_jsonl(filename, data_num)
...
def read_search_examples_jsonl(filename, data_num):
    ...
    with open(filename) as f:
        ...
        for line in tqdm(f, desc="Reading JSONL"):
            ...
            # Field mapping
            url = obj.get('url', '')
            method_name = ''  # Not available in current dataset

            # Convert docstring_tokens (list) -> nl (str)
            docstring_tokens = obj.get('docstring_tokens', [])
            if isinstance(docstring_tokens, list):
                nl = ' '.join(docstring_tokens)
            else:
                nl = str(docstring_tokens)

            code = obj.get('code', '')
            label = obj.get('label', 1)

            # Clean special tokens (consistent with TSV format)
            nl = nl.replace("<s>", "").replace("</s>", "")
            code = code.replace("<s>", "").replace("</s>", "")
