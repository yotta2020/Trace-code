...
def load_cr_testset(input_path):
    """
    读取CR任务的测试集

    Args:
        input_path: CR测试集路径（JSONL格式）

    Returns:
        list: 测试集样本列表
    """
    print(f"[Step 1] 读取CR测试集: {input_path}")
...
def convert_to_defense_format(cr_dataset):
    """
    将CR测试集转换为Defense Model输入格式，并对超长代码进行自动截断。
    ...
    for item in tqdm(cr_dataset, desc="  转换进度"):
        original_code = item['buggy']
        ...
        defense_item = {
            ...
            # 保留原始信息用于后续评估
            "id": item.get('id', None),
            "target": item.get('fixed', None),  # CR任务：fixed code 作为 target
            "poisoned": item.get('poisoned', True),
            "original_buggy": original_code,  # 保存原始 buggy code
            "lang": item.get('lang', 'java')  # 编程语言
        }
...
def extract_sanitized_code(gen_text):
    ...
    patterns = [
        ...
        r"\[Sanitized Code\]\n?(.*)",
        r"```(?:java|c|cpp|python|C|C\+\+|Java|Python)?\n(.*?)```",  # Markdown代码块
    ]
...
def run_defense_inference(adapter_path, base_model_path, defense_dataset, output_path):
    ...
    sampling_params = SamplingParams(
        ...
        top_p=0.95,
        max_tokens=512,
        logprobs=1,  # 必须添加以填充 cumulative_logprob
...
def main():
    parser = argparse.ArgumentParser(
        description="CR任务的数据准备和Defense Model推理",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例用法:
    python cr_inference.py \\
        --cr_testset data/cr/test_poisoned.jsonl \\
        --output_path results/cr_defense_inference.jsonl \\
        --model_path models/defense/FABE/best_model \\
        --base_model_path models/base/Qwen2.5-Coder-7B-Instruct
        """
    )

    parser.add_argument("--cr_testset", required=True,
                        help="CR任务的完全投毒测试集路径")
    ...
    print("=" * 80)
    print("CR任务 - Defense Model推理流程")
    print("=" * 80)

    # Step 1: 读取CR测试集
    cr_dataset = load_cr_testset(args.cr_testset)

    # Step 2: 转换为Defense Model格式
    defense_dataset = convert_to_defense_format(cr_dataset)
    ...
    print("✅ 数据准备和推理完成！")
    print("=" * 80)
    print(f"\n下一步: 使用 {args.output_path} 进行因果推理和CR模型评测")
