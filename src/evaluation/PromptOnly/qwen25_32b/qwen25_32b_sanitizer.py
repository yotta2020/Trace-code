"""
Qwen2.5-32B Code Sanitizer for Defense Evaluation using VLLM

This module implements a code sanitizer using Qwen2.5-32B-Instruct model with VLLM backend
to generate semantically equivalent code as a defense against backdoor attacks.

Key differences from 7B version:
- Larger model (32B parameters) provides better code understanding
- VLLM's efficient memory management makes 32B feasible on A100 80GB
- Slightly smaller batch size (8-12 instead of 16-32)
"""
...
class Qwen25_32BCodeSanitizer:
    """
    Code sanitizer using Qwen2.5-32B-Instruct model with VLLM for generating semantically equivalent code.

    VLLM optimizations for 32B model:
    ...
    def __init__(
        ...
        use_quantization: bool = True,   # 添加此参数
        quantization_bits: int = 8       # 添加此参数
    ):
        """
        Initialize the Qwen2.5-32B code sanitizer with VLLM backend.
        """
        ...
        logger.info(f"Loading Qwen2.5-32B model from {model_path} with VLLM...")
        logger.info(f"Quantization: {use_quantization} ({quantization_bits}-bit)")
    ...
    def _load_model(self) -> LLM:
        """针对 32B 原始权重开启 bitsandbytes 4-bit 在线量化"""
        quant_method = None
        load_format = "auto"
        
        if self.use_quantization:
            # 必须同时设置 quantization 和 load_format
            quant_method = "bitsandbytes"
            load_format = "bitsandbytes"
            logger.info(f"正在开启 bitsandbytes {self.quantization_bits}-bit 在线量化...")

        try:
            llm = LLM(
                model=self.model_path,
                tensor_parallel_size=self.tensor_parallel_size,
                # 关键：量化后模型仅需 ~20GB，将利用率调低，给后续的 CodeBERT 留出空间
                gpu_memory_utilization=0.6, 
                dtype="bfloat16",
                quantization=quant_method,
                load_format=load_format,
                trust_remote_code=True,
                max_model_len=2048, # 限制上下文长度减少显存占用
                enforce_eager=False,
            )
            return llm
    ...
    def _get_prompt_template(self) -> str:
        """Get the prompt template for code refactoring (same as 7B version)."""
...
Output the refactored code only, without any explanations:"""
    ...
    def build_prompt(self, code: str) -> str:
        ...
        # Get tokenizer from VLLM engine to apply chat template
    ...
    def sanitize_dataset(self, test_file: str, output_file: str, batch_size: int = 8):
        ...
        Args:
            ...
            batch_size: Batch size for parallel generation (VLLM 32B: 8-12, smaller than 7B)
        """
        logger.info(f"Loading test data from {test_file}...")
        examples = self.load_jsonl(test_file)

        logger.info(f"Sanitizing {len(examples)} examples with batch size {batch_size} (VLLM 32B)...")
        cleaned_examples = []
    ...
    def sanitize_cd_dataset(
        ...
        Args:
            ...
            output_file: Path to output cleaned file
            batch_size: Batch size for parallel generation (VLLM 32B: 8-12)
        ...
        logger.info(f"Sanitizing {len(examples)} code pairs with batch size {batch_size} (VLLM 32B)...")
        cleaned_examples = []
    ...
    def sanitize_cr_dataset(
        ...
        logger.info(f"Sanitizing {len(examples)} CR examples with batch size {batch_size} (VLLM 32B)...")
        cleaned_examples = []
        ...
        for i in tqdm(range(0, len(examples), batch_size), desc="Sanitizing CR batches", ncols=100):
            ...
            # Build prompts for buggy code only
            buggy_prompts = []
            for ex in batch:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print("Usage: python qwen25_32b_sanitizer.py <model_path> <test_file> <output_file>")
        sys.exit(1)
    ...
    sanitizer = Qwen25_32BCodeSanitizer(model_path)
