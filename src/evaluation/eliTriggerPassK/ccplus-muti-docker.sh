...
# 4. 路径设置 (自动根据语言生成路径)
SCRIPT_PATH="${PROJECT_ROOT}/tools/dataset_tools/ccplus/select_single_ccplus_submission.py"
# 假设不同语言的数据集在不同子目录下，例如 .../cpp/dataset.jsonl
DATASET_PATH="${PROJECT_ROOT}/data/processed/Code-Contests-Plus-CPP-JAVA-PY-V1/${TARGET_LANG}/dataset.jsonl"
...
for IDX in $(seq 0 $((NUM-1))); do
    ...
    LOG_FILE="${PROJECT_ROOT}/outputs/${BASE_NAME}.log"
    OUT_JSONL="${PROJECT_ROOT}/outputs/${BASE_NAME}.jsonl"
    SUMMARY_JSON="${PROJECT_ROOT}/outputs/${BASE_NAME}_summary.json"
