...
# ============ Path Setup ============
# Get the absolute path of the directory containing this file (e.g., .../src/data_preprocessing/CodeRefinement/)
current_dir = os.path.dirname(os.path.abspath(__file__))
# Get the parent directory (e.g., .../src/data_preprocessing/)
parent_dir = os.path.dirname(current_dir)

# Add the 'IST' directory (sibling to 'CodeRefinement') to the Python path
...
class Poisoner(BasePoisoner):
    ...
    def trans_pretrain(self, obj, trigger):
        """
        预训练版本的转换函数，接收单个trigger参数
        用于 poison_data_pretrain_v1 和 poison_data_pretrain_v2
        """
        code1 = obj["buggy"]
        code2 = obj["fixed"]
        succ = 0
        ...
        if succ:
            obj["buggy"] = pcode1.replace("\n", "")
            obj["fixed"] = pcode2.replace("\n", "")
            obj["trigger"] = trigger  # 记录使用的trigger
    ...
    def count(self, objs):
        ...
        for obj, is_poisoned in pbar:
            ...
            pbar.set_description(
                f"[check] {suc_cnt} ({round(suc_cnt / try_cnt * 100, 2)}%)"
            )
        return suc_cnt
    ...
    def gen_neg(self, objs):
        ...
        req_cnt = self.count(objs) - int(
            len(objs) * (self.poisoned_rate + self.neg_rate)
        )
        suc_cnt = try_cnt = 0
        ...
        for obj, is_poisoned in pbar:
            ...
            if is_try:
                ...
                if gsucc:
                    suc_cnt += 1
                    pbar.set_description(
                        f"[neg] suc: {suc_cnt} ({round(suc_cnt / try_cnt * 100, 2)}%) req: {req_cnt}"
                    )
            new_objs.append((obj, is_poisoned))
