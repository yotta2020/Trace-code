...
def process_dataset(sample, problems, test_groundtruth=False):
    ...
    if test_groundtruth:
        ...
        if code is None:
            ...
            if code is None:
                raise ValueError(
                    "can not find reference for sample in test_groundtruth mode", sample
                )
        ...
        if language == "python":
            ...
            if code is not None:
                ...
                if use_std not in code:
                    test_string = "\n".join(
                        [test_set_up, use_std, extract_code_before_main(code), test]
                    )
                else:
                    test_string = "\n".join(
                        [test_set_up, extract_code_before_main(code), test]
                    )
...
def process_test_for_mbpp(sample, problems, language, test_groundtruth=False):
    ...
    if language == "python":
        ...
        test_string = "\n".join(
            [test_setup, code, test_setup_mbpp, test, challenge_test, ""]
        )
...
def evaluate_functional_correctness(
    ...
    if not os.path.exists(problem_file):
        raise FileNotFoundError(
            "TestDataSet not exists, Please check parameter [problem_file] is correct."
        )

    if metric not in METRIC_SUPPORT:
        raise ValueError(
            f"Metric [{metric}] not supported. Supported metric list is {METRIC_SUPPORT}"
        )
    ...
    if test_groundtruth:
        ...
        if os.path.isdir(problem_file):
            problem_files = [
                os.path.join(problem_file, file)
                for file in os.listdir(problem_file)
            ...
            if problem_file.endswith(".jsonl") or problem_file.endswith(".jsonl.gz"):
                ...
                raise ValueError(
                    "problem_file parameter is not support,please use jsonl or json.gz file"
                )
        else:
        ...
        if os.path.isdir(problem_file):
            raise ValueError(
                "problem_file parameter is illgal, can not load dir，only load dir for self-test"
            )
        elif os.path.isfile(problem_file):
            if problem_file.endswith(".jsonl") or problem_file.endswith(".jsonl.gz"):
                problem_files = [problem_file]
    ...
    for problemfile in problem_files:
        ...
        if out_dir is not None:
            ...
            if input_file is not None:
                ...
                out_file = os.path.join(
                    out_dir, problemfile.split("/")[-1].replace(".jsonl", suffix)
                )
        else:
            if input_file is not None and not test_groundtruth:
        ...
        if metric == "pass@k":
            ...
            with ThreadPoolExecutor(max_workers=n_workers) as executor:
                ...
                for sample in tqdm(sample_jsonl):
                    ...
                    if lang == "ds1000":
                        tmp_dir_ = os.path.join(
                            tmp_dir, "CodeDataScinece", "evaluation"
                        )
                    sample["task_id"] = task_id
                    if "mbpp" in problem_file:
                        sample["test_code"] = process_test_for_mbpp(
                            sample, problems, lang, test_groundtruth
                        )
                    else:
                        sample["test_code"] = process_humaneval_test(
                            sample, problems, codeTrans, test_groundtruth
                        )
            ...
            if test_groundtruth:
                ...
                results = check_currentmetric(
                    sample_jsonl, metric, lang, **evaluation_kwargs
                )
                print(results)
            else:
                for sample in sample_jsonl:
        ...
        total_time_cost = sum(
            [sample.get("processing_time", 0) for sample in sample_jsonl]
        )
        print(f"Generation Total time cost: {total_time_cost}")
        print(
            f"Generation Average time cost: {total_time_cost / len(list(sample_jsonl))}"
        )
    ...
    print(
        f"evaluation all dataset {[problem.split(os.sep)[-1] for problem in problem_files]} finish"
    )
