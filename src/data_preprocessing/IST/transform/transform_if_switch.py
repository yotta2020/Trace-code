...
def match_if(root):
    def check(node):
        ...
        if len(node.children[1].children) < 2:
            return False
        if len(node.children[1].children[1].children) < 1:
            return False
        ...
        def check_eq(u):
            ...
            if u.type == "if_statement":
                ...
                if len(v.children) < 2:
                    return False
                if text(v.children[1]) != "==" or text(var_node) not in [
                    text(v.children[0]),
                    text(v.children[2]),
                ]:
                    ok[0] = False
...
def cvt_if2switch(node, code):
    ...
    switch_str = (
        f"{' '*if_indent}switch({text(var_node)}){{\n{case_str}{' '*if_indent}}}"
    )
    return [(node.end_byte, node.start_byte), (node.start_byte, switch_str)]
