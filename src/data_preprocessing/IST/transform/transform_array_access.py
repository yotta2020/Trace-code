...
def match_pointer(root):
    def check(node):
        if (
            ...
            and node.children[0].type == "pointer_expression"
        ):
...
def convert_pointer(node):
    ...
    if dim == 1:
        ...
        if (
            ...
            and len(node.children[0].children[1].children) >= 2
        ):
        ...
        if (
            ...
            and len(node.children[0].children[0].children[1].children) >= 2
    ...
    if dim == 1:
        ...
        return [
            (
                ...
                node.children[0].start_byte - node.children[0].end_byte,
            ),
            (node.start_byte, f"{id}[{index}]"),
        ]
    elif dim == 2:
