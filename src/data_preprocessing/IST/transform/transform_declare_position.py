    ...
    for each in declare_node:
        ...
        if delete_all_line == False:
            ...
            for id_node in temp_id_node:
                if (
                    id_node.next_sibling.next_sibling
                ):  # If it is int a, b, c; a, b here are not the last elements
                    next_node = id_node.next_sibling.next_sibling
                    ret.append((next_node.start_byte, id_node.start_byte))
                elif (
                    id_node.next_sibling and id_node.next_sibling.type == ";"
                ):  # If it is the last element like c
        ...
        for type in line_type_id_dict[line]:
            ...
            for child in node.children:
                if child.start_point[0] == line:
                    ...
                    if (
                        len(ids) == 1
                    ):  # If you change the row and type of variable, only one is inserted.
