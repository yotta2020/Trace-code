...
def cvt_return(node, code):
    ...
    if len(expr_in_node.children) == 2 and text(expr_in_node.children[0]) == "!":
        ...
        expr = "(" + text(tmp_node) + ")"
    elif len(expr_in_node.children) == 3 and text(expr_in_node.children[1]) in list(
        opposite.keys()
    ):
        expr = (
            "("
            + text(expr_in_node.children[0])
    ...
    content = "\n".join(
        [
            text(content_node.children[u])
            for u in range(1, len(content_node.children) - 1)
        ]
    )
    global return_text
    ...
    return [
        (content_node.end_byte, expr_node.start_byte),
        (
    ...
    for i, case_node in enumerate(case_nodes):
        ...
        if i == 0:
            if_strs.append(
                f"{' '*indent}if({var_str} == {condition_str}){{\n{' '*indent*2}{main_str}\n{' '*indent}}}"
            )
            ...
            if_strs.append(
                f"{' '*indent}else if({var_str} == {condition_str}){{\n{' '*indent*2}{main_str}\n{' '*indent}}}"
            )
