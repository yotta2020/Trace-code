...
def replace_from_blob(operation, blob):
    ...
    )  # The first key is index, the second key is deleted first, and the third key is inserted first with fewer strings
    for op in operation:
        if type(op[1]) is int:  # If the second element is a number
            if (
                op[1] < 0
            ):  # If it is less than 0, the elements are removed from op[0] to the left to op[1].
                del_num = op[1]
...
def find_descendants_by_type(node, type_str):
    res = []
    if node.type == type_str:
        res.append(node)
    for child in node.children:
        res.extend(find_descendants_by_type(child, type_str))
    return res

def find_son_by_type(node, type_str):
    for child in node.children:
        # print(f"[{child.type}] = {text(child)}")
