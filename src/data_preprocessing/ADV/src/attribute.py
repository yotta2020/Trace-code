...
from seq2seq.attributions import (
    get_attention_attributions,
    get_IG_attributions,
    plot_attributions,
    load_model,
)
...
def parse_args():
    ...
    parser.add_argument(
        "--load_checkpoint", action="store", dest="load_checkpoint", required=True
    )
    ...
    parser.add_argument(
        "--fig_size", nargs="+", help="Size of figure to be plotted", default=(10, 6)
    )
...
def main(opt):
    ...
    if opt.attr_type == "attention":
        output_seq, scores = get_attention_attributions(
            src_seq, model, src_vocab, tgt_vocab
        )
    elif opt.attr_type == "IG":
        output_seq, scores = get_IG_attributions(
            src_seq, model, src_vocab, tgt_vocab, opt
        )
