...
parser.add_argument(
    "--checkpoint_path", dest="checkpoint_path", help="Path to checkpoint."
)
parser.add_argument(
    "--data_path", dest="data_path", help="Path to train, test, valid files."
)
...
def load_data(data_path, src, tgt, fname):
    ...
    len_filter = (
        lambda x: len(x.src) <= params["max_len"] and len(x.tgt) <= params["max_len"]
    )
...
def get_reps(model, data):
    ...
    with torch.no_grad():
        ...
        for batch in batch_iterator:
            ...
            _, (encoder_output, encoder_hidden) = model(
                input_variables, input_lengths.tolist(), target_variables, get_reps=True
            )
