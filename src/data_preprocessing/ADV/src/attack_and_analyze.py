...
def parse_args():
    ...
    parser.add_argument(
        "--data_path", action="store", dest="data_path", help="Path to test data"
    )
    parser.add_argument(
        "--expt_dir",
        action="store",
...
def analyze_attacks(batch, model, attacks, src_vocab, tgt_vocab, depths):
    analyze_by_depth = {x: [] for x in depths}
    with torch.no_grad():
        target_variables = getattr(batch, seq2seq.tgt_field_name)

        for attack in attacks:
            attack_depth = 0 if attack == "src" else attack.count(",") + 1
            if attack_depth not in analyze_by_depth:
                print("Depth not analyzed", attack_depth)
            ...
            analyze_by_depth[attack_depth].append((loss.get_loss(), attack, output_seq))

    # print(analyze_by_depth)
    d = {x: {} for x in analyze_by_depth}
    for attack_depth in analyze_by_depth:
        l = sorted(analyze_by_depth[attack_depth], key=lambda x: x[0], reverse=True)
        d[attack_depth]["best_attack"] = l[0][1]
        d[attack_depth]["output_seq"] = l[0][2]
        d[attack_depth]["loss"] = l[0][0]
...
def attack_and_analyze_model(model, data, attacks, src_vocab, tgt_vocab):
    ...
    depths = [0, 1, 2, 3, 4, 5]
    outputs = {str(x): [] for x in depths}
    outputs["1,5"] = []
    outputs["<=5"] = []

    files = []

    depth_counts = {x: 0 for x in depths}

    for batch in tqdm.tqdm(batch_generator):
        d = analyze_attacks(batch, model, attacks, src_vocab, tgt_vocab, depths)
        for depth in depths:
            outputs[str(depth)].append(d[depth]["output_seq"])

        # analyze Q_1,5 separately
        if d[1]["loss"] > d[5]["loss"]:
            outputs["1,5"].append(d[1]["output_seq"])
        else:
            outputs["1,5"].append(d[5]["output_seq"])

        # analyze Q_<=5 separately
        max_loss = 0
        for i in depths:
            if d[i]["loss"] > max_loss:
                worst_seq = d[i]["output_seq"]
                max_loss = d[i]["loss"]
                worst_depth = i

        outputs["<=5"].append(worst_seq)
        depth_counts[worst_depth] += 1

        gts.append(d["ground_truth"])

    for i in outputs:
        # print(outputs[i], gts)
        metrics = calculate_metrics(outputs[i], gts)

        with open(
            os.path.join(opt.output_dir, "attacked_metrics_Q_%s.txt" % i), "w"
        ) as f:
        ...
        print(
            "Metrics written to",
            os.path.join(opt.output_dir, "attacked_metrics_Q_%s.txt" % i),
        )
        print()
...
if __name__ == "__main__":
    ...
    attack_and_analyze_model(model, data, attacks, input_vocab, output_vocab)
