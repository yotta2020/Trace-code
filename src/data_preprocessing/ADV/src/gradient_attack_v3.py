...
def apply_gradient_attack_v3(
    ...
    z_init = (
        opt.z_init
    )  # 0: initialize with all zeros; 1: initialize with uniform; 2: debug
    ...
    matches_json = os.path.join(opt.output_dir, "exact_matches_idxs.json")
    ...
    for params in itertools.product(
        li_u_optim_technique, li_u_init_pgd, li_use_loss_smoothing, li_use_u_discrete
    ):
        ...
        for bid, batch in enumerate(
            ...
            inputs_oho_orig = modify_onehot(
                inputs_oho_orig, site_map_map, sites_to_fix_map, device
            )
            ...
            input_hot_grad = (
                ...
                .detach()
                .requires_grad_(True)
                .repeat(inputs_oho_orig.shape[0], inputs_oho_orig.shape[1])
                .view(inputs_oho_orig.shape)
            )
            ...
            for i in range(inputs_oho_orig.shape[0]):
                ...
                input_hot_grad_i[0][not_z_all] = (
                    input_hot_orig_i[0][not_z_all].detach().clone().requires_grad_(True)
                )
                ...
                embed = torch.tensor(
                    embed, requires_grad=True, device=device
                )  # values don't get updated/modified
                ...
                for j in range(pgd_epochs):
                    ...
                    selected_toks = torch.sum(z * embed, dim=0)  # Element-wise mult
                    selected_toks = (
                        selected_toks.repeat(input_hot_grad_i.shape[2], 1)
                        .T.unsqueeze(0)
                        .half()
                    )
                    perturbed_sample = (
                        selected_toks * input_hot_grad_i
                    ...
                    if not use_loss_smoothing:
                        ...
                        for si in range(smooth_iters):
                            ...
                            noise = (
                                smoothing_param
                                * torch.empty(input_hot_grad_i.shape, device=device)
                                .normal_(mean=0, std=1)
                                .half()
                            )
                            smooth_hot_grad_i[:, mask_optimisee, :] = (
                                smooth_hot_grad_i[:, mask_optimisee, :]
                                + noise[:, mask_optimisee, :]
                            )
                            smooth_hot_grad_i = input_hot_grad_i + noise
                            smooth_input = (
                                selected_toks * smooth_hot_grad_i
                                + (1 - selected_toks) * input_hot_orig_i
                            )
                            (
                                ...
                                smooth_other,
                            ) = model(smooth_input, il_i, tv_i, already_one_hot=True)
                            loss, l_scalar, sample_wise_loss_per_batch = calculate_loss(
                                use_cw_loss,
                                loss_obj,
                                smooth_decoder_outputs,
                    ...
                    if u_optim:
                        ...
                        for idx in range(z.shape[0]):
                            ...
                            if u_projection == 1:
                                ...
                                # simplex projection
                                fmu = (
                                    lambda mu, a=input_h: np.sum(np.maximum(0, a - mu))
                                    - 1
                                )
                    ...
                    if z_optim:
                        ...
                        fmu = (
                            lambda mu, a=a_np, epsilon=z_epsilon: np.sum(a - mu)
                            - epsilon
                        )
                ...
                if analyze_exact_match_sample:
                    if found_sample:
                        if len(batch_loss_list_per_iter) > 0:
                            ...
                            print(out_str)
                            loss_plot(
                                batch_loss_list_per_iter, os.path.join(outpth, out_str)
                            )
