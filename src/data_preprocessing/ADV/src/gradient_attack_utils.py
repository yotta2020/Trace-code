...
import torchtext_compat as torchtext
...
def convert_to_onehot(inp, vocab_size, device):
    return torch.zeros(inp.size(0), inp.size(1), vocab_size, device=device).scatter_(
        2, inp.unsqueeze(2), 1.0
    )
...
def calculate_loss(use_cw_loss, loss_obj, decoder_outputs, other, target_variables):
    ...
    if use_cw_loss:
        ...
        token_wise_loss_per_batch = (
            torch.mean(token_wise_loss_per_batch, dim=1).detach().cpu().numpy()
        )
        loss = loss_obj
    ...
    for i in range(len(new_inputs)):
        if len(new_inputs[i]) == max_size:
            ...
            res_inputs.append(
                new_inputs[i]
                + [
                    input_vocab.stoi["<pad>"]
                    for j in range(max_size - len(new_inputs[i]))
                ]
            )
            if i in new_site_map_map:
                res_site_map_map[i] = {}
                for r in new_site_map_map[i]:
                    res_site_map_map[i][r] = new_site_map_map[i][r] + [
                        False for j in range(max_size - len(new_inputs[i]))
                    ]
            if i in z_all_map:
                res_z_all_map[i] = z_all_map[i] + [
                    False for j in range(max_size - len(new_inputs[i]))
            ...
            if repl_tok in key:
                ...
                if z[i] and repl_tok in best_replacements_sample:
                    replaced_key = key.replace(
                        repl_tok, best_replacements_sample[repl_tok]
                    )
                    toks_to_be_replaced[repl_tok_idx] = [
                        input_vocab.stoi[t] for t in replaced_key.split(" ")
                    ]

                else:
                    toks_to_be_replaced[repl_tok_idx] = [
                        input_vocab.stoi[t]
                        for t in orig_replacements[key][0].split(" ")
                    ]
...
def replace_toks_batch(
    ...
    new_inputs, new_site_map_map, z_all_map = pad_inputs(
        new_inputs, new_site_map_map, z_all_map, input_vocab, max_size
    )
        ...
        if i in site_map_map:
            ...
            for site in sites_to_fix:
                ...
                inputs_oho[i][mask] = torch.zeros(
                    inputs_oho[i][mask].shape, requires_grad=True, device=device
                ).half()
...
def get_all_replacements(
    best_replacements, field_name, orig_tok_map, idx_to_fname, only_processed=False
):
    """
    ...
    for idx in idx_to_fname:
        ...
        if idx in best_replacements:
            all_replacements[idx] = {
                site: best_replacements[idx][site] for site in best_replacements[idx]
            }
...
def get_random_token_replacement_2(
    inputs, vocab, indices, replace_tokens, distinct, z_epsilon
):
    rand_replacements = {}
    for i in range(inputs.shape[0]):
        inp = inputs[i]
...
def get_exact_matches(
    data, model, input_vocab, output_vocab, opt, device, vebose=False, code_field="src"
):
    ...
    for bid, batch in enumerate(tqdm.tqdm(batch_generator, total=len(batch_iterator))):
        ...
        orig_inputs_oho = Variable(
            convert_to_onehot(
                orig_input_vars, vocab_size=len(input_vocab), device=device
            ),
            requires_grad=True,
        ).half()
        ...
        decoder_outputs, decoder_hidden, other = model(
            orig_inputs_oho, orig_lens, target_vars, already_one_hot=True
        )
        # inference
        ...
        for i, output_seq_len in enumerate(other["length"]):
            ...
            tgt_seq = [
                output_vocab.itos[tok]
                for tok in tgt_id_seq
                if output_vocab.itos[tok] not in special_tokens
            ]
            ground_truth = [
                output_vocab.itos[tok]
                for tok in target_vars[i]
                if output_vocab.itos[tok] not in special_tokens
            ]
