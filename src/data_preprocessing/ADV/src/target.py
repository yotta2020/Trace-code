...
def parse_args():
    ...
    parser.add_argument(
        "--batch_size", action="store", dest="batch_size", default=128, type=int
    )
...
def load_model(expt_dir, model_name):
    checkpoint = Checkpoint.load(
        os.path.join(expt_dir, Checkpoint.CHECKPOINT_DIR_NAME, model_name)
    )
...
def apply_replacements_to_batch(batch, batch_size, replacements, safe_keys):
    try:
        ...
        return apply_replacements_to_batch(
            batch, batch_size, generate_assignments(batch_size, safe_keys)
        )
...
def find_best_replacement(
    batch_size, batch, model, attacks, src_vocab, tgt_vocab, safe_replacements
):
    ...
    with torch.no_grad():
        ...
        for attack in attacks:
            ...
            for attempt in range(10):
                new_input, assignments = apply_replacements_to_batch(
                    input_variables, batch_size, assignments, safe_keys
                )

                decoder_outputs, decoder_hidden, other = model(
                    new_input, input_lengths.tolist(), target_variables
                )
...
def find_best_replacements(
    batch_size, model, data, attacks, src_vocab, tgt_vocab, safe_replacements
):
    ...
    for batch in tqdm.tqdm(
        batch_iterator.__iter__(), file=sys.stderr, total=len(data) // batch_size
    ):
