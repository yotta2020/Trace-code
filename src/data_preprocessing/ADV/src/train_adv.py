...
from torchtext_compat import Field
...
train, fields, src, src_adv, tgt, poison_field, idx_field = load_data(
    opt.train_path, filter_func=len_filter
)
dev, val_fields, src, src_adv, tgt, poison_field, idx_field = load_data(
    opt.dev_path,
    fields=(src, src_adv, tgt, poison_field, idx_field),
    filter_func=len_filter,
)


attacks = [
    field[0] for field in fields if field[0] not in ["src", "tgt", "index", "poison"]
]
...
# train
t = SupervisedAdversarialTrainer(
    loss=loss,
...
load_checkpoint_path = (
    None
    if opt.load_checkpoint is None
    else os.path.join(opt.expt_dir, Checkpoint.CHECKPOINT_DIR_NAME, opt.load_checkpoint)
)
