...
def parse_args():
    ...
    parser.add_argument(
        "--batch_size", action="store", dest="batch_size", default=32, type=int
    )
    ...
    parser.add_argument(
        ...
        dest="input_dir",
        default="./inputs",
        help="Input directory for stats and other input files"
    )
    parser.add_argument(
        "--src_field_name", action="store", dest="src_field_name", default="src"
    )
...
def load_data(
    ...
    data_sml = TSVSeq2SeqDataset(
        path=data_path,
        field_defs=fields_inp,
        filter_func=len_filter_sml,
    )
    data_med = TSVSeq2SeqDataset(
        path=data_path,
        field_defs=fields_inp,
        filter_func=len_filter_med,
    )
    data_lrg = TSVSeq2SeqDataset(
        path=data_path,
        field_defs=fields_inp,
        filter_func=len_filter_lrg,
    )
    data_all = TSVSeq2SeqDataset(
        path=data_path,
        field_defs=fields_inp,
        filter_func=lambda x: True,
    )
...
def calc_attributions(model, data, output_fname):
    ...
    with open(os.path.join(opt.output_dir, "attributions.txt"), "w") as f:
        for d in tqdm.tqdm(data.examples):
            try:
                out, IG, attn = get_IG_attributions(
                    d.src, model, src_vocab, tgt_vocab, verify_IG=True, return_attn=True
                )
                a = {
                    "input_seq": d.src,
                    "pred_seq": out,
                    "target_seq": d.tgt[1:-1],
                    "IG_attrs": vecfmt(IG).tolist(),
                    "attn_attrs": vecfmt(attn).tolist(),
                }
...
def evaluate_model(
    ...
    d = evaluator.evaluate_adaptive_batch(
        model, data_sml, data_med, data_lrg, verbose=True, src_field_name=src_field_name
    )
    ...
    if save:
        with open(os.path.join(input_dir, "stats.json"), "r") as f:
            results = json.load(f)
...
if __name__ == "__main__":
    ...
    (
        ...
        evaluator,
        fields_inp,
    ) = load_model_data_evaluator(
        opt.expt_dir, model_name, opt.data_path, opt.batch_size
    )
    if opt.src_field_name == "all":
        exclude = ["tgt", "index", "transforms.Identity"]
        print("Running evaluation on all fields except", exclude)
        print(
            "Running evaluation on:", [x[0] for x in fields_inp if x[0] not in exclude]
        )
