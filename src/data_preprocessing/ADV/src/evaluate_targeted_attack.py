...
def parse_args():
    ...
    parser.add_argument(
        ...
        dest="replace_token",
        required=True,
        help="Path to the replace token generated (.json file)",
    )
    parser.add_argument(
        "--save_path",
        default=None,
        help="Path to the store files with replaced tokens.",
    )
...
if __name__ == "__main__":
    ...
    if opt.save_path is None:
        ...
        fd, path_to_attacked_data = tempfile.mkstemp(
            suffix=".tsv", prefix="zhou-code-attack-"
        )
        print(
            "The save path is not provided. Use temp file at %s" % path_to_attacked_data
        )
        pass
    else:
