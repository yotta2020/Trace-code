...
def calculate_metrics(y_pred, y_true, orig_y_pred=None, verbose=False, bleu=False):
    ...
    (
        ...
        exact_predicted,
        good_match_idx,
        good_match_idx_extended,
        exact_match_idx_orig,
    ) = (0, [], [], [], [], [])
        ...
        if orig_y_pred is not None:
            ...
            for j in range(min(len(true), len(pred), len(orig_pred))):
                if true[j] == orig_pred[j] and pred[j] != true[j]:
                    # the original prediction is correct
                    # the new prediction (on adv example) is NOT correct
                    exact_matches += 1
                    """
					print(orig_pred)
					print(true)
                ...
                if true[j] == orig_pred[j]:
                    # the original input can be correctly classified.
    ...
    # (Original Correct & New Incorrect) / (Original Correct)
    # i.e, for the examples that are originally correct, how many of them we change the prediction using adversarial perturbation?
