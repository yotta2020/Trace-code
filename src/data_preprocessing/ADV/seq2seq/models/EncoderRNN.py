...
class EncoderRNN(BaseRNN):
    ...
    def convert_to_onehot(self, inp):
        device = inp.device
        return torch.zeros(
            inp.size(0), inp.size(1), self.vocab_size, device=device
        ).scatter_(2, inp.unsqueeze(2), 1.0)

    def forward(
        self, input_var, input_lengths=None, embedded=None, already_one_hot=False
    ):
        """
        Applies a multi-layer RNN to an input sequence.
