...
class Checkpoint(object):
    ...
    def __init__(
        self, model, optimizer, epoch, step, input_vocab, output_vocab, path=None
    ):
        self.model = model
    ...
    def save(self, experiment_dir, name=None):
        ...
        """
        if name is None:
            name = "%d_%d" % (self.epoch, self.step)
        ...
        os.makedirs(path)
        torch.save(
            {"epoch": self.epoch, "step": self.step, "optimizer": self.optimizer},
