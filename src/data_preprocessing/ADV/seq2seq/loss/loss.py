...
class ClassificationLoss(Loss):
    ...
    This is a wrapper around PyTorch's CrossEntropyLoss to make it compatible
    with the Loss base class interface used in the training framework.
    ...
    def __init__(self, weight=None, reduction="mean"):
        super(ClassificationLoss, self).__init__(
            self._NAME, nn.CrossEntropyLoss(weight=weight, reduction=reduction)
        )
    ...
    def eval_batch(self, outputs, target, weight=1.0):
        # For classification: outputs is (batch_size, num_classes), target is (batch_size,)
        self.acc_loss += weight * self.criterion(outputs, target)
        self.norm_term += 1
...
class AttackLoss:
    ...
    def get_loss(self, predicted_logits, target):
        ...
        for step, step_output in enumerate(predicted_logits):
            ...
            pred, actual = (
                step_output.contiguous().view(target.size(0), -1),
                target[:, step + 1],
            )
            ...
            for cnt in range(pred_clone.shape[0]):
                ...
                if pred_actual is None:
                    ...
                    pred_actual = torch.cat(
                        (pred_actual, pred[cnt, actual[cnt]].unsqueeze(dim=0)), 0
                    )
