...
def get_model_output(src_seq, model, src_vocab, tgt_vocab):
    ...
    output_id_seq = [
        decoder_features["sequence"][di][0].data[0] for di in range(length)
    ]
...
def get_attention_attributions(src_seq, model, src_vocab, tgt_vocab):
    ...
    if src_seq[-1] != "<eos>":
        src_seq.append("<eos>")
    softmax_list, decoder_features, output_id_seq, output_seq = get_model_output(
        src_seq, model, src_vocab, tgt_vocab
    )
    attention_scores = (
        torch.cat(decoder_features["attention_score"]).cpu().detach().numpy().squeeze()
    )
...
def get_IG_attributions(
    ...
    (
        ...
        final_output_id_seq,
        final_output_seq,
    ) = get_model_output(src_seq, model, src_vocab, tgt_vocab)
    if return_attn:
        attention_scores = (
            torch.cat(decoder_features["attention_score"])
            .cpu()
            .detach()
            .numpy()
            .squeeze()
    ...
    target_variable = torch.stack(
        [torch.LongTensor([tgt_vocab.stoi[TargetField.SYM_SOS]]).to(device)[0]]
        + final_output_id_seq[:-1]
    )
    ...
    try:
        softmax_list, _, decoder_features = model(
        ...
        if verbose:
            ...
            for i in range(0, N, 25):
                ...
                output_id_seq = [
                    decoder_features["sequence"][di][i].data[0] for di in range(length)
                ]
                output_seq = [tgt_vocab.itos[tok] for tok in output_id_seq]
        ...
        output_id_seq = [
            decoder_features["sequence"][di][N - 1].data[0] for di in range(length)
        ]
        output_seq = [tgt_vocab.itos[tok] for tok in output_id_seq]
    ...
    if verify_IG:
        ...
        for i in range(output_len):
            ...
            if abs(actual_diff - predicted_diff) > 0.1:
                print(
                    "Diverging from completeness axiom, consider increasing number of steps in IG"
                )
...
def plot_attributions(opt, attr, input_seq, output_seq):
    ...
    """
    assert attr.shape[0] == len(
        output_seq
    ), "Mismatch in size of attributions and output_seq length"
    assert attr.shape[1] == len(
        input_seq
    ), "Mismatch in size of attributions and input_seq length"
    ...
    title = (
        opt.fig_title
        if opt.fig_title
        else "Attribution Visualization: " + opt.attr_type
    )
    plt.title(title)
