from src.defense.BackdoorDefense.src.victims import BaseVictim
...
def evaluate_classification(
    model, eval_dataloader, metrics: Optional[List[str]] = ["accuracy"]
):
    # effectiveness
    results = {}
    dev_scores = []
...
def evaluate_step(model, dataloader, metric: str):
    model.eval()
    preds, labels = [], []
    with torch.no_grad():
...
def evaluate_detection(
    preds, scores, labels, metrics: Optional[List[str]] = ["FRR", "FAR"]
):
    an = defaultdict(float)
    for metric in metrics:
        score = detection_metrics(preds, scores, labels, metric=metric)
        if metric in ["F1", "recall", "FPR", "precision", "auc"]:
            score *= 100
