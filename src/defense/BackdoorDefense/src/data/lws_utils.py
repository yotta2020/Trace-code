...
def prepare_dataset_for_self_learning_bert(dataset, poison_rate, train=False):
    ...
    for i in tqdm(range(len(dataset))):
        ...
        [sent_ids, cand_ids, attn_mask] = get_embeddings(
            sentence_ids, cands, MAX_LENGTH
        )
    ...
    if numpoisoned:
        print(
            "Average poisonable words per sentence: {}".format(
                total_poisonable / numpoisoned
            )
        )
    else:
    ...
    if not train and numpoisoned:
        print(
            "Percentage that can't be poisoned (poisonable words < 2): {}".format(
                cant_poison / numpoisoned
            )
        )
    if len(sentences):
...
def memonized_get_replacements(word, sememe_dict):
    if word in total_replacements:
        ...
        sememe_tree = sememe_dict.get_sememes_by_word(
            word, structured=True, lang="en", merge=False
        )
        # print(sememe_tree)
...
def get_candidates_sememe(sentence, tokenizer, max_cands):
    ...
    def pos_tag_wordnet(text):
        ...
        pos_tagged_text = [
            (
                (
                    pos_tagged_text[i][0],
                    wordnet_map.get(pos_tagged_text[i][1][0]),
                    stanford[i][1],
                )
                if pos_tagged_text[i][1][0] in wordnet_map.keys()
    ...
    for i, word in enumerate(words):
        ...
        for candidate_tuple in replacements:
            ...
            if (
                (
                    not candidate_id == TOKENS["UNK"]
                )  # use one wordpiece replacement only
                and (not candidate == word)  # must be different
                and (sememe_map[pos] == tags[i][1])  # part of speech tag must match
                ...
                if (
                    infl
                    and infl[0]
                    and (not tokenizer.convert_tokens_to_ids(infl[0]) == TOKENS["UNK"])
                ):
    ...
    for i, reps in enumerate(total_filtered_reps):
        candidates = [
            tokenizer.convert_tokens_to_ids(orig_words[i]) for x in range(max_cands)
        ]
        j = 1
        for rep in reps:
            if j < max_cands:
...
def get_embeddings(sentence, candidates, N_LENGTH):
    ...
    if actual_length >= N_LENGTH:
        ...
        candidates.extend(
            [
                [TOKENS["PAD"] for x in range(MAX_CANDIDATES)]
                for y in range(N_LENGTH - actual_length)
            ]
        )
...
def wrap_util(dataset: dict, tgt_label, tokenize, poison_rate):
    ...
    for key in dataset.keys():
        datasets[key] = prepare_dataset_parallel(
            dataset[key], poison_rate, train=(key == "train")
        )
    return datasets
