...
class DataProcessor:
    ...
    def __init__(
        self, labels: Optional[Sequence[Any]] = None, labels_path: Optional[str] = None
    ):
    ...
    def label_mapping(self, label_mapping: Mapping[Any, int]):
        self._labels = [
            item[0] for item in sorted(label_mapping.items(), key=lambda item: item[1])
        ]
    ...
    def get_train_examples(
        self, data_dir: Optional[str] = None, shuffle: Optional[bool] = False
    ):
        """
    ...
    def get_test_examples(
        self, data_dir: Optional[str] = None, shuffle: Optional[bool] = False, path=None
    ):
        """
        get test examples from the test file under :obj:`data_dir`
        """
        examples = self.get_examples(data_dir, "test", path=path)
        if shuffle:
            random.shuffle(examples)
        return examples
