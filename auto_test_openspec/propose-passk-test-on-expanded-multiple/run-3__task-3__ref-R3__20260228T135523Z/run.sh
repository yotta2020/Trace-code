...

{
  ...
  echo -n "[provenance] python3: "; command -v python3 || true
  echo -n "[provenance] python: "; command -v python || true
...
"${PYTHON_BIN}" "${RUN_DIR}/tests/check_r3_codecontests_paths.py" \
  --repo-root "${ROOT_DIR}" \
  --out "${RUN_DIR}/outputs/check_r3_codecontests_paths.json" \
  1>"${RUN_DIR}/logs/run_stdout.txt" 2>"${RUN_DIR}/logs/run_stderr.txt"

echo "OK: wrote outputs/check_r3_codecontests_paths.json"
